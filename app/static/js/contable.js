...
async function consultarComparativoClientesSiigo(event) {
    ...
    try {
        ...
        const resumenGrupo = (titulo, total) => `<div class="recent-section" style="flex:1; min-width:280px;"><h4 style="margin-top:0;">${titulo}</h4><p class="form-help">Saldos consolidados al ${escapeSiigo(data.fecha_cartera)}.</p><div class="stats-grid"><div class="stat-card"><h3>Facturacion</h3><p class="stat-number" style="font-size:1.35rem;">${formatoSiigoNumero(total.facturacion)}</p></div><div class="stat-card"><h3>Cartera pendiente</h3><p class="stat-number" style="font-size:1.35rem;">${formatoSiigoNumero(total.cartera)}</p></div><div class="stat-card"><h3>Saldo a favor</h3><p class="stat-number" style="font-size:1.35rem;">${formatoSiigoNumero(total.saldo_favor)}</p></div></div></div>`;

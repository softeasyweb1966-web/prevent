...
function tablaCarteraClientesSiigo(clientes) {
    ...
    const filas = clientes.map((cliente, index) => {
        return `<tr><td>${escapeSiigo(cliente.identificacion)}</td><td>${escapeSiigo(cliente.cliente)}</td><td>${formatoSiigoNumero(cliente.facturado)}</td><td>${formatoSiigoNumero(cliente.recaudado)}</td><td>${formatoSiigoNumero(cliente.ajustes_ac)}</td><td>${formatoSiigoNumero(cliente.notas_credito)}</td><td>${formatoSiigoNumero(cliente.notas_debito)}</td><td>${formatoSiigoNumero(cliente.por_vencer)}</td><td>${formatoSiigoNumero(cliente.vencido_1_30)}</td><td>${formatoSiigoNumero(cliente.vencido_31_60)}</td><td>${formatoSiigoNumero(cliente.vencido_61_90)}</td><td>${formatoSiigoNumero(cliente.vencido_91_mas)}</td><td>${formatoSiigoNumero(cliente.saldo)}</td><td><button type="button" class="action-btn" data-siigo-cliente-index="${index}">Ver detalle</button></td></tr>`;
    }).join('');
    return `<h4 style="margin:20px 0 8px;">Detalle de cartera por cliente</h4><p class="form-help">Seleccione “Ver detalle” para consultar las facturas que componen cada saldo.</p><div class="siigo-tabla-con-encabezado-fijo"><table class="data-table"><thead><tr><th>Identificacion</th><th>Cliente</th><th>Facturado</th><th>Recaudado</th><th>Ajustes AC</th><th>Notas crédito</th><th>Notas débito</th><th>Por vencer</th><th>1 a 30</th><th>31 a 60</th><th>61 a 90</th><th>Mas de 90</th><th>Saldo</th><th>Detalle</th></tr></thead><tbody>${filas}</tbody></table></div>`;
}
...
function crearPanelCarteraDinamicaSiigo(tipo) {
    ...
    panel.innerHTML = esPagos
        ...
        : `<h3 style="margin-top:0;">Cartera y recaudo por periodo</h3><p class="form-help">Calculado desde las facturas FV, recibos RC, ajustes AC y notas crédito/débito asociados a cada factura. Saldo = facturado - recibos RC - ajustes AC - notas crédito + notas débito. Los ajustes incluyen las retenciones que afectan cartera; un ajuste negativo aumenta el saldo. La fecha de vencimiento corresponde a la cuota indicada por SIIGO.</p><form data-siigo-cartera="recaudo"><div class="form-row"><div class="form-group"><label>Fecha de corte</label><input type="date" required value="${new Date().toISOString().slice(0, 10)}"></div><div class="form-group" style="align-self:end;"><button class="btn btn-primary" type="submit">Generar cartera</button></div></div></form><div class="table-container" style="margin-top:16px;"></div>`;
...
async function consultarCarteraDinamicaSiigo(form, tipo) {
    ...
    try {
        ...
        if (tipo === 'vencidas') {
            ...
            const tablaPeriodos = rows.length ? `<div class="siigo-tabla-con-encabezado-fijo"><table class="data-table"><thead><tr><th>Periodo</th><th>Facturado</th><th>Recaudado</th><th>Ajustes AC</th><th>Notas crédito</th><th>Notas débito</th><th>Por vencer</th><th>1 a 30</th><th>31 a 60</th><th>61 a 90</th><th>Mas de 90</th><th>Saldo</th></tr></thead><tbody>${rows.map(item => `<tr><td>${item.periodo}</td><td>${formatoSiigoNumero(item.facturado)}</td><td>${formatoSiigoNumero(item.recaudado)}</td><td>${formatoSiigoNumero(item.ajustes_ac)}</td><td>${formatoSiigoNumero(item.notas_credito)}</td><td>${formatoSiigoNumero(item.notas_debito)}</td><td>${formatoSiigoNumero(item.por_vencer)}</td><td>${formatoSiigoNumero(item.vencido_1_30)}</td><td>${formatoSiigoNumero(item.vencido_31_60)}</td><td>${formatoSiigoNumero(item.vencido_61_90)}</td><td>${formatoSiigoNumero(item.vencido_91_mas)}</td><td>${formatoSiigoNumero(item.saldo)}</td></tr>`).join('')}</tbody></table></div>` : 'No se encontraron facturas para la fecha seleccionada.';
